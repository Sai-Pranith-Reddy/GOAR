...
public class ShowPath : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (a != null && b != null && NavigationController.instance.IsCurrentlyNavigating())
        {
            if (path.status == NavMeshPathStatus.PathPartial || path.status == NavMeshPathStatus.PathInvalid)
            {
                ...
                ToastManager.Instance.ShowToast("Problem calculating route");
    ...
    IEnumerator DrawPath(NavMeshPath path)
    {
        ...
        if (path.status == NavMeshPathStatus.PathComplete && b != null)
        {
            AudioNavigationManager.Instance.UpdateNavigation(path.corners, b.position);
            Debug.Log($"Path analysis: {path.corners.Length} corners");
        }
        else if (path.status == NavMeshPathStatus.PathPartial)
        {
            Debug.LogWarning("Partial path - audio might be incomplete");
            AudioNavigationManager.Instance.UpdateNavigation(path.corners, b.position);
        }
        if (path.status == NavMeshPathStatus.PathComplete && b != null)
        {
            AudioNavigationManager.Instance.UpdateNavigation(path.corners, b.position);
            Debug.Log($"Path updated with {path.corners.Length} corners");
        }
    }
